...
class ScheduleController {
  get_date_schedule = (req, res) => {
    let sql =
      "select `schedule`.`id`, `schedule`.`number_lesson`, `schedule`.`date_lesson`, `schedule`.`group_id`, `lessons`.`name`, `teac_1`.`fullname` as `teacher_first`, `teac_2`.`fullname` as `teacher_second`, `schedule`.`room_first`, `schedule`.`room_second` from `schedule` join `lessons` on `schedule`.`lessons_id` = `lessons`.`id` join `teachers` as `teac_1` on `schedule`.`teachers_id_first` = `teac_1`.`id` left JOIN `teachers` as `teac_2` on `schedule`.`teachers_id_second` = `teac_2`.`id` where `schedule`.`date_lesson` = ? and `schedule`.`group_id` = ?;";
  ...
  get_date_schedule_min = (req, res) => {
    let sql =
      "select `schedule`.`id`, `schedule`.`number_lesson`, `lessons`.`name` from `schedule` join `lessons` on `schedule`.`lessons_id` = `lessons`.`id` where `schedule`.`date_lesson` = ? and `schedule`.`group_id` = ? ORDER BY `schedule`.`number_lesson`;";
    let values = [req.query.date_lesson, req.query.group_id];
    if (validators.everyFiled(values, res)) {
      return res.status(400).json({
        name: "None felids",
  ...
  post_add_schedule = (req, res) => {
    let sql =
      "insert into `schedule` (group_id, date_lesson, room_first, number_lesson, lessons_id, teachers_id_first) value (?, ?, ?, ?, (select `lessons`.`id` from `lessons` where `lessons`.`name` like ? limit 1), (SELECT `teachers`.`id` FROM `teachers` WHERE `teachers`.`fullname` LIKE ? LIMIT 1))";
    let values = [
      req.body.group_id,
  ...
  put_update_schedule = (req, res) => {
    let sql =
      "update `schedule` set `room_first` = ?, `lessons_id` = (select `lessons`.`id` from `lessons` where `lessons`.`name` like ? limit 1), `teachers_id_first` = (SELECT `teachers`.`id` FROM `teachers` WHERE `teachers`.`fullname` LIKE ? LIMIT 1) where `schedule`.`id` = ?;";
    let values = [
      req.body.room_first,
      req.body.lesson_name,
      req.body.teachers_fullname_first,
  ...
  get_find_id_schedule = (req, res) => {
    let sql = "select `schedule`.`id`, `lessons`.`name`, `teac_1`.`fullname` as `teacher_first`, `teac_2`.`fullname` as `teacher_second`, `schedule`.`room_first`, `schedule`.`room_second` from `schedule` join `lessons` on `schedule`.`lessons_id` = `lessons`.`id` join `teachers` as `teac_1` on `schedule`.`teachers_id_first` = `teac_1`.`id` left JOIN `teachers` as `teac_2` on `schedule`.`teachers_id_second` = `teac_2`.`id` WHERE `schedule`.`id` = ?;";
    let values = [req.query.id];
    if (validators.everyFiled(values, res)) {
      return res.status(400).json({
        name: "None felids",
        message: "Что-то осталось не отправленным",
      });
    }
    let callback = (err, result) => {
      if (err) {
